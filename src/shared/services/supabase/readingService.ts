...
import { CreateQuestion } from "@/shared/types/model/question.types";
import {
  ReadingContent,
  ReadingContentWithAudio,
} from "@/shared/types/model/reading.types";

class ReadingService extends BaseService<ReadingContent> {
  constructor() {
    super("reading_content");
  }

  // Override getAll to include audio assets
  async getAll(): Promise<ReadingContentWithAudio[]> {
    ...
    return data as ReadingContentWithAudio[];
  }

  // Override getById to include audio assets
  async getById(id: string | number): Promise<ReadingContentWithAudio | null> {
  ...
  async createQuestions(readingId: string, questions: CreateQuestion[]) {
  ...
  async updateQuestions(readingId: string, questions: CreateQuestion[]) {
  ...
  async getReadingDetailById(
    id: string | number
  ): Promise<ReadingContentWithAudio | null> {
    const { data, error } = await this.supabase
      .from(this.tableName)
      .select(
        `
  ...
  async getTextsByLevel(level: CEFRLevel) {
    const { data, error } = await this.supabase
      .from(this.tableName)
      .select(
        `
  ...
  async getTextCountByLevel(level: CEFRLevel) {
